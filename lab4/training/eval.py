...
from train_and_test import qerror_loss, get_batch_job
...
def model_eval(ctx: EncodeContext, model, eval_start, eval_end):
    ...
    for batch_idx in range(eval_start, eval_end):
        ...
        cost_loss, cost_loss_median, cost_loss_max, cost_max_idx = qerror_loss(estimate_cost, target_cost,
                                                                               ctx.cost_label_min, ctx.cost_label_max)
        card_loss, card_loss_median, card_loss_max, card_max_idx = qerror_loss(estimate_cardinality, target_cardinality,
                                                                               ctx.card_label_min, ctx.card_label_max)
        ...
        target_cards = torch.cat(
            [target_cards, unnormalize(target_cardinality, ctx.card_label_min, ctx.card_label_max)])

    batch_num = eval_end - eval_start
    print("evaluating cost loss: {}, evaluating card loss: {}".format(cost_loss_total / batch_num,
