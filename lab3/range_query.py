...
if __name__ == '__main__':
    sql = "select * from imdb.title where c1>1 and c1<2 and c2>3 and c2<4 and c3 < 4 and c3 > 10"
    db = "mysql"
    table = "test"
    col_left = None
    col_right = None
    a = ParsedRangeQuery(sql, db, table, col_left, col_right)
    b = a.parse_range_query(sql)
    print(b.column_names())
