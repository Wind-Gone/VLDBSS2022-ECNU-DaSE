...
class LeafNode():
    ...
    def estimate(self, range_query):
        ...
        # print(col_name)
        # print(range_query.query)
        # print(range_query.col_left)
        if self.scope.n_rows() == 0:
            return 0

        if col_name not in range_query.col_left.keys():
            return 1
        return 1.0 * self.hist.between_row_count(range_query.col_left[col_name], range_query.col_right[col_name],
                                                 self.scope.n_rows()) / self.scope.n_rows()
        pass
...
class SPN:
    ...
    def split_rows(dataset, scope):
        ...
        # YOUR CODE HERE
        part_set = []
        for i in scope.row_idxs:
            part_set.append(dataset[i])
        labels = KMeans(n_clusters=2, random_state=0, n_init=20, init='random').fit_predict(part_set)
        left_idx = []
        right_idx = []

        assert (scope.n_rows() == len(labels))

        for i, v in enumerate(labels):
            if v == 0:
                left_idx.append(scope.row_idxs[i])
            else:
                right_idx.append(scope.row_idxs[i])
        return NodeScope(left_idx, scope.col_idxs), NodeScope(right_idx, scope.col_idxs)
    ...
    def get_next_op(scope, row_batch_threshold, split_col_failed):
        ...
        if split_col_failed == True and scope.n_rows() > row_batch_threshold:
